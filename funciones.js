function jugar() {
    ponerBG();
    var audioInicio = new Audio('sfx/start.m4a');
    var margenCarga = 700;

    function cargarSiguientePantalla() {
        window.location.assign('personaje.html');
    }

    audioInicio.addEventListener('loadedmetadata', function() {
        var tiempoRestante = Math.max((audioInicio.duration * 1000) - margenCarga, 0);
        setTimeout(cargarSiguientePantalla, tiempoRestante);
    });

    audioInicio.play().catch(function(){
        // Si el navegador bloquea la reproduccion, seguimos con la navegacion.
    });
...
function reproducirMusicaJuego(){
    ...
    musicaJuego = new Audio('sfx/juego.m4a');
...
function conteo(){
    ...
    var capaConteo = document.querySelector('.conteo');
    if (capaConteo) {
        capaConteo.setAttribute('onclick', '');
    }
...
function disparo1(){
    ...
    reproducirAudio('sfx/disparo.m4a');
}
